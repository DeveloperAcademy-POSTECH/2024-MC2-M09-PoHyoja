...
extension PhotoViewModel {    
    ...
    func syncPhoto(of userName: String?) async {
        ...
        do {
            // 원격 및 로컬 데이터 가져오기
            async let remoteData = remoteRepository.fetchPhotos(userName)
            async let localData = localRepository.fetchPhotos()
    ...
    func uploadPhoto(of user: User, imgData: Data) async throws {
        ...
        // 1. 원격 사진 데이터 업로드
        let urlString = try await remoteRepository.uploadPhotoData(of: user.name, photo: photo)
        
        do {
            // 2. 원격 사진 정보 저장
            try await remoteRepository.addPhoto(photo, urlString: urlString)
            ...
            try await remoteRepository.deletePhotoData(of: urlString)
        ...
        do {
            // 4. 로컬 저장
            try await localRepository.addPhoto(photo)
    ...
    func downloadPhoto(of urlString: String) async throws -> Data {
        try await remoteRepository.downloadPhotoData(of: urlString)
    }
    ...
    func delete(_ photo: Photo) async throws {
        ...
        do {
            // 2-1. 원격 우선 삭제
            try await remoteRepository.deletePhoto(of: photo.id)
            
            // 2-2. 로컬 삭제
            // (SOT - 원격) : 원격 성공, 로컬 실패 시에는 UI 복구 -> sync에서 해결!
            try await localRepository.deletePhoto(photo.id)
    ...
    func updatePhoto(_ photo: Photo) async {
        ...
        do {
            ...
            try await remoteRepository.updatePhoto(photo)
            try await localRepository.updatePhoto(photo)
        } catch {
